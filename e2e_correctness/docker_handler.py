...
def start_memgraph_mage_container(
    image_name: str, container_name: str, port: int
) -> str:
    """
    ...
    command_start_memgraph = f"docker run \
                        ...
                        --log-level=TRACE \
                        --telemetry-enabled=False"
    memgraph_started = subprocess.run(
        command_start_memgraph, shell=True, stdout=subprocess.PIPE
    )
...
def stop_container(container_id: str) -> bool:
    ...
    docker_stoped = subprocess.run(
        command_stop_container, shell=True, stdout=subprocess.PIPE
    )
    try:
