...
class Constants:
    BATCH_SIZE = 1000
    CONFIG_FILE_PATH = (
        "/var/lib/memgraph/internal_modules/migrate_config/mysql_config.yaml"
    )
    CURSOR = "cursor"
    DATABASE = "database"
...
def init_query(
    ...
    """
    Prepare MySQL cursor for batch streaming.
    ...
    connection = _get_mysql_connection(resolved_config)
    ...
    # Stash per-thread
    mysql_dict[thread_id][Constants.DRIVER] = connection
    mysql_dict[thread_id][Constants.CURSOR] = cursor
    mysql_dict[thread_id][Constants.RESULT] = cursor
...
def query(
    ...
    global mysql_dict
    thread_id = threading.get_native_id()
    cursor = mysql_dict[thread_id][Constants.RESULT]
    ...
    for _ in range(Constants.BATCH_SIZE):
        try:
            ...
            if row is None:
                break
            batch.append(mgp.Record(row=_mysql_to_primitive(row, cursor.description)))
...
def cleanup_query():
    ...
    cursor = mysql_dict[thread_id].get(Constants.CURSOR)
    if cursor:
        try:
            cursor.close()
        except Exception as e:
    ...
    connection = mysql_dict[thread_id].get(Constants.DRIVER)
    if connection:
        try:
            connection.close()
        except Exception as e:
...
def test_connection(
    ...
    """
    Test MySQL connection using configuration.
    ...
    result = _test_mysql_connection(connection, resolved_config)
...
def list_tables(
    driver_config: mgp.Nullable[mgp.Any] = None,
) -> mgp.Record(table_name=str, table_type=str, engine=str):
    """
    List all table names in the MySQL database.

    Args:
        driver_config: Either a map with config or string name to load from YAML file

    Returns:
        One record per table with name, type, and engine
    """
    # Resolve driver configuration
    resolved_config = _resolve_driver_config(driver_config)
    ...
    return [
        mgp.Record(table_name=table[0], table_type=table[1], engine=table[2])
        for table in tables
    ]
...
def describe_table(
    table_name: str,
    driver_config: mgp.Nullable[mgp.Any] = None,
) -> mgp.Record(column_name=str, data_type=str, is_nullable=str, column_default=str, column_key=str):
    """
    Describe table structure in MySQL database.
    ...
    cursor = connection.cursor()
    cursor.execute(describe_query, [table_name, database])
    ...
    return [
        mgp.Record(
            ...
            column_key=col[4] if col[4] is not None else ""
        )
        for col in columns
    ]
...
def add_configuration(
    ...
    """
    Add or update MySQL configuration in YAML file.

    Args:
        configuration_name: Name of the configuration to save
        driver_config: MySQL connection configuration
...
def _mysql_to_primitive(row, description):
    """Convert MySQL row to primitive types."""
    result = {}
    for i, column in enumerate(description):
        column_name = column[0]  # MySQL description is tuple (name, type, ...)
        value = row[i]
        ...
        if value is None:
            ...
            try:
                ...
                result[column_name] = base64.b64encode(value).decode("ascii")
        else:
...
def _get_mysql_connection(driver_config: Dict[str, Any]) -> mysql_connector.MySQLConnection:
    """
    Get MySQL connection.
    ...
    charset = driver_config.get(Constants.CHARSET, "utf8mb4")
    collation = driver_config.get(Constants.COLLATION, "utf8mb4_unicode_ci")
...
def _test_mysql_connection(
    connection: mysql_connector.MySQLConnection, driver_config: Dict[str, Any]
) -> str:
    """Test MySQL connection."""
    try:
        ...
        if result and result[0] == 1:
            ...
            return "MySQL connection test failed"
    except Exception as e:
