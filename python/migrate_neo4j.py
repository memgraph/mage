...
class Constants:
    BATCH_SIZE = 1000
    CONFIG_FILE_PATH = (
        "/var/lib/memgraph/internal_modules/migrate_config/neo4j_config.yaml"
    )
    CURSOR = "cursor"
    DATABASE = "database"
...
def init_query(
    ...
    """
    Prepare Neo4j cursor for batch streaming.
    ...
    driver = _get_neo4j_driver(resolved_config)
    database = resolved_config.get(Constants.DATABASE, None)
...
def query(
    ...
    for _ in range(Constants.BATCH_SIZE):
        try:
            ...
            if record is None:
                break
            batch.append(mgp.Record(row=_neo4j_to_primitive(record)))
...
def test_connection(
    ...
    """
    Test Neo4j connection using configuration.
    ...
    result = _test_neo4j_connection(driver, resolved_config)
...
def list_relationship_types(
    ...
    RETURN relationshipType as relationship_type,
           size([()-[r:relationshipType]->() | r]) as count
...
def describe_label(
    ...
    """
    Describe properties of a specific node label in Neo4j database.
    ...
    result = session.run(describe_query)
...
def add_configuration(
    ...
    """
    Add or update Neo4j configuration in YAML file.

    Args:
        configuration_name: Name of the configuration to save
        driver_config: Neo4j connection configuration
...
def _neo4j_to_primitive(record):
    """Convert Neo4j record to primitive types."""
    result = {}
    for key, value in record.items():
        result[key] = _convert_neo4j_value(value)
    return result
...
def _get_neo4j_driver(driver_config: Dict[str, Any]) -> GraphDatabase.driver:
    """
    Get Neo4j driver.
    ...
    uri_scheme = driver_config.get(Constants.URI_SCHEME, "bolt")
    uri = f"{uri_scheme}://{host}:{port}"
    
    # Get credentials
    username = driver_config.get(Constants.USERNAME, "neo4j")
    password = driver_config.get(Constants.PASSWORD, "password")
    
    return GraphDatabase.driver(uri, auth=(username, password))


def _test_neo4j_connection(
    driver: GraphDatabase.driver, driver_config: Dict[str, Any]
) -> str:
    """Test Neo4j connection."""
    try:
        # Test connection with a simple query
        database = driver_config.get(Constants.DATABASE, None)
        if database:
            session = driver.session(database=database)
        else:
            session = driver.session()
        ...
        if test_value == 1:
            ...
            return "Neo4j connection test failed"
    except Exception as e:
