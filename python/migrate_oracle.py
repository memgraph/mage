...
class Constants:
    BATCH_SIZE = 1000
    CONFIG_FILE_PATH = (
        "/var/lib/memgraph/internal_modules/migrate_config/oracle_config.yaml"
    )
    CURSOR = "cursor"
    DATABASE = "database"
...
def init_query(
    ...
    """
    Prepare Oracle cursor for batch streaming.
    ...
    if limit_val is not None:
        try:
            ...
            query = f"SELECT * FROM ({query}) WHERE ROWNUM <= {limit_val}"
        except Exception:
    ...
    # Stash per-thread
    oracle_dict[thread_id][Constants.DRIVER] = connection
    oracle_dict[thread_id][Constants.CURSOR] = cursor
    oracle_dict[thread_id][Constants.RESULT] = cursor
...
def query(
    ...
    global oracle_dict
    thread_id = threading.get_native_id()
    cursor = oracle_dict[thread_id][Constants.RESULT]
    ...
    for _ in range(Constants.BATCH_SIZE):
        try:
            ...
            if row is None:
                break
            batch.append(mgp.Record(row=_oracle_to_primitive(row, cursor.description)))
...
def cleanup_query():
    ...
    cursor = oracle_dict[thread_id].get(Constants.CURSOR)
    if cursor:
        try:
            cursor.close()
        except Exception as e:
    ...
    connection = oracle_dict[thread_id].get(Constants.DRIVER)
    if connection:
        try:
            connection.close()
        except Exception as e:
    ...
    oracle_dict.pop(thread_id, None)
...
def test_connection(
    ...
    """
    Test Oracle connection using configuration.
    ...
    result = _test_oracle_connection(connection, resolved_config)
...
def list_tables(
    driver_config: mgp.Nullable[mgp.Any] = None,
) -> mgp.Record(table_name=str, table_type=str):
    """
    List all table names in the Oracle database.

    Args:
        driver_config: Either a map with config or string name to load from YAML file

    Returns:
        One record per table with name and type
    """
    # Resolve driver configuration
    resolved_config = _resolve_driver_config(driver_config)
    ...
    # Get username for schema filtering
    username = resolved_config.get(Constants.USERNAME, "").upper()
    ...
    return [
        mgp.Record(table_name=table[0], table_type=table[1])
        for table in tables
    ]
...
def describe_table(
    table_name: str,
    driver_config: mgp.Nullable[mgp.Any] = None,
) -> mgp.Record(column_name=str, data_type=str, nullable=str, data_length=int):
    """
    Describe table structure in Oracle database.
    ...
    cursor = connection.cursor()
    cursor.execute(describe_query, [table_name.upper()])
    ...
    return [
        mgp.Record(
            ...
            data_type=col[1],
            nullable=col[2],
            data_length=col[3] if col[3] is not None else 0
        )
        for col in columns
    ]
...
def add_configuration(
    ...
    """
    Add or update Oracle configuration in YAML file.

    Args:
        configuration_name: Name of the configuration to save
        driver_config: Oracle connection configuration
...
def _oracle_to_primitive(row, description):
    """Convert Oracle row to primitive types."""
    result = {}
    for i, (column_name, data_type, display_size, internal_size, precision, scale, null_ok) in enumerate(description):
        value = row[i]
...
def _get_oracle_connection(driver_config: Dict[str, Any]) -> oracledb.Connection:
    """
    Get Oracle connection.
    ...
    service_name = driver_config.get(Constants.SERVICE_NAME)
    sid = driver_config.get(Constants.SID)
    ...
    return oracledb.connect(user=username, password=password, dsn=dsn, **config)


def _test_oracle_connection(
    connection: oracledb.Connection, driver_config: Dict[str, Any]
) -> str:
    """Test Oracle connection."""
    try:
        ...
        if result and result[0] == 1:
            ...
            return "Oracle connection test failed"
    except Exception as e:
