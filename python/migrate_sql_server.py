...
class Constants:
    BATCH_SIZE = 1000
    CONFIG_FILE_PATH = (
        "/var/lib/memgraph/internal_modules/migrate_config/sql_server_config.yaml"
    )
    CURSOR = "cursor"
    DATABASE = "database"
...
def init_query(
    ...
    """
    Prepare SQL Server cursor for batch streaming.
    ...
    connection = _get_sql_server_connection(resolved_config)
    ...
    if limit_val is not None:
        try:
            ...
            query = f"SELECT TOP {limit_val} * FROM ({query}) AS subquery"
        except Exception:
    ...
    # Stash per-thread
    sql_server_dict[thread_id][Constants.DRIVER] = connection
    sql_server_dict[thread_id][Constants.CURSOR] = cursor
    sql_server_dict[thread_id][Constants.RESULT] = cursor
...
def query(
    ...
    global sql_server_dict
    thread_id = threading.get_native_id()
    cursor = sql_server_dict[thread_id][Constants.RESULT]
    ...
    for _ in range(Constants.BATCH_SIZE):
        try:
            ...
            if row is None:
                break
            batch.append(mgp.Record(row=_sql_server_to_primitive(row, cursor.description)))
...
def cleanup_query():
    ...
    cursor = sql_server_dict[thread_id].get(Constants.CURSOR)
    if cursor:
        try:
            cursor.close()
        except Exception as e:
    ...
    connection = sql_server_dict[thread_id].get(Constants.DRIVER)
    if connection:
        try:
            connection.close()
        except Exception as e:
    ...
    sql_server_dict.pop(thread_id, None)
...
def test_connection(
    ...
    """
    Test SQL Server connection using configuration.
    ...
    result = _test_sql_server_connection(connection, resolved_config)
...
def list_tables(
    ...
    """
    List all table names in the SQL Server database.
...
def describe_table(
    table_name: str,
    driver_config: mgp.Nullable[mgp.Any] = None,
) -> mgp.Record(column_name=str, data_type=str, is_nullable=str, column_default=str, character_maximum_length=int):
    """
    Describe table structure in SQL Server database.
    ...
    cursor = connection.cursor()
    cursor.execute(describe_query, [table_name])
    ...
    return [
        mgp.Record(
            ...
            data_type=col[1],
            is_nullable=col[2],
            column_default=col[3] if col[3] is not None else "",
            character_maximum_length=col[4] if col[4] is not None else 0
        )
        for col in columns
    ]
...
def add_configuration(
    ...
    """
    Add or update SQL Server configuration in YAML file.

    Args:
        configuration_name: Name of the configuration to save
        driver_config: SQL Server connection configuration
...
def _sql_server_to_primitive(row, description):
    """Convert SQL Server row to primitive types."""
    result = {}
    for i, column in enumerate(description):
        column_name = column[0]  # SQL Server description is tuple (name, type, ...)
        value = row[i]
...
def _get_sql_server_connection(driver_config: Dict[str, Any]) -> pyodbc.Connection:
    """
    Get SQL Server connection.
    ...
    driver_name = driver_config.get(Constants.DRIVER_NAME, "ODBC Driver 17 for SQL Server")
    trusted_connection = driver_config.get(Constants.TRUSTED_CONNECTION, False)
    encrypt = driver_config.get(Constants.ENCRYPT, True)
...
def _test_sql_server_connection(
    connection: pyodbc.Connection, driver_config: Dict[str, Any]
) -> str:
    """Test SQL Server connection."""
    try:
        ...
        if result and result[0] == 1:
            return "SQL Server connection successful"
        else:
            return "SQL Server connection test failed"
    except Exception as e:
        return f"SQL Server connection failed: {str(e)}"
