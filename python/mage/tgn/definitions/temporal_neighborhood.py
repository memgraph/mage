...
class TemporalNeighborhood:
    ...
    def update_neighborhood(self, sources, destinations, edge_idx, timestamps):
        ...
        self.neighborhood = {**{node: [] for node in set(sources).union(set(destinations))}, **self.neighborhood}
        for neighbor in zip(sources, destinations, edge_idx, timestamps):
            self.neighborhood[neighbor[0]].append((neighbor[1:]))
            self.neighborhood[neighbor[1]].append((neighbor[0], neighbor[2], neighbor[3]))
    ...
    def get_neighborhood(self, node: int, timestamp: int, num_neighbors: int):
        ...
        if node not in self.neighborhood:
            return np.zeros(num_neighbors, dtype=int), np.zeros(num_neighbors, dtype=int), np.zeros(num_neighbors)
        ...
        neighbors = np.append(arr=neighbors, values=np.zeros(num_neighbors-len(neighbors)))
        edge_idxs = np.append(arr=edge_idxs, values=np.zeros(num_neighbors - len(edge_idxs)))
        timestamps = np.append(arr=timestamps, values=np.zeros(num_neighbors - len(timestamps)))
