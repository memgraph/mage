...
class ChainChunkFactory:
    @staticmethod
    @validate(Parameter.POPULATION_SIZE, Parameter.NUM_OF_PROCESSES)
    def create(
        graph: Graph, parameters: Dict[str, Any] = None
    ) -> Optional[List[Population]]:
        """Returns a list of num_of_processes populations that have approximately
        population_size / num_of_processes individuals."""
        ...
        chunks = ChainChunkFactory._list_chunks(
            individuals, population_size, num_of_processes
        )

        for i, chunk in enumerate(chunks):
            prev_chunk = i - 1 if i - 1 > 0 else num_of_processes - 1
            next_chunk = i + 1 if i + 1 < num_of_processes else 0
            populations.append(
