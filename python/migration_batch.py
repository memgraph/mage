...
def init_migrate_mysql(
    ...
    if 1 not in mysql_dict:
        mysql_dict[1] = {}


    if Constants.CURSOR not in mysql_dict[1]:
        mysql_dict[1][Constants.CURSOR] = None

    if mysql_dict[1][Constants.CURSOR] is None:
        ...
        mysql_dict[1][Constants.CONNECTION] = connection
        mysql_dict[1][Constants.CURSOR] = cursor
        mysql_dict[1][Constants.COLUMN_NAMES] = [column[Constants.I_COLUMN_NAME] for column in cursor.description]
...
def migrate_mysql(
    ...
    global mysql_dict
    #print("migrate")
    cursor = mysql_dict[1][Constants.CURSOR]
    column_names = mysql_dict[1][Constants.COLUMN_NAMES]
    results = []
...
def cleanup_migrate_mysql():
    ...
    #print("cleanup")
    mysql_dict[1][Constants.CURSOR] = None
    mysql_dict[1][Constants.CONNECTION].close()
    mysql_dict[1][Constants.CONNECTION].commit()
...
def init_migrate_sql_server(
    ...
    if sql_server_dict[threading.get_native_id][Constants.CURSOR] is None:
        ...
        sql_server_dict[threading.get_native_id][Constants.COLUMN_NAMES] = [column[Constants.I_COLUMN_NAME] for column in cursor.description]
...
def init_migrate_oracle_db(
    ...
    if oracle_db_dict[threading.get_native_id][Constants.CURSOR] is None:
        ...
        oracle_db_dict[threading.get_native_id][Constants.COLUMN_NAMES] = [column[Constants.I_COLUMN_NAME] for column in cursor.description]
...
mgp.add_batch_read_proc(migrate_oracle_db, init_migrate_oracle_db, cleanup_migrate_oracle_db)