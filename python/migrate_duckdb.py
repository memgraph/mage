...
class Constants:
    BATCH_SIZE = 1000
    CONFIG_FILE_PATH = (
        "/var/lib/memgraph/internal_modules/migrate_config/duckdb_config.yaml"
    )
    CURSOR = "cursor"
    DATABASE = "database"
    DRIVER = "driver"
...
def init_query(
    ...
    """
    Prepare DuckDB cursor for batch streaming.
    ...
    connection = _get_duckdb_connection(resolved_config)
    ...
    # Stash per-thread
    duckdb_dict[thread_id][Constants.DRIVER] = connection
    duckdb_dict[thread_id][Constants.CURSOR] = cursor
    duckdb_dict[thread_id][Constants.RESULT] = cursor
...
def query(
    ...
    global duckdb_dict
    thread_id = threading.get_native_id()
    cursor = duckdb_dict[thread_id][Constants.RESULT]
    ...
    for _ in range(Constants.BATCH_SIZE):
        try:
            ...
            if row is None:
                break
            batch.append(mgp.Record(row=_duckdb_to_primitive(row, cursor.description)))
...
def cleanup_query():
    ...
    cursor = duckdb_dict[thread_id].get(Constants.CURSOR)
    if cursor:
        try:
            cursor.close()
        except Exception as e:
    ...
    connection = duckdb_dict[thread_id].get(Constants.DRIVER)
    if connection:
        try:
            connection.close()
        except Exception as e:
...
def test_connection(
    ...
    """
    Test DuckDB connection using configuration.
    ...
    result = _test_duckdb_connection(connection, resolved_config)
...
def describe_table(
    ...
    """
    Describe table structure in DuckDB database.
...
def add_configuration(
    ...
    """
    Add or update DuckDB configuration in YAML file.

    Args:
        configuration_name: Name of the configuration to save
        driver_config: DuckDB connection configuration
...
def _duckdb_to_primitive(row, description):
    """Convert DuckDB row to primitive types."""
    result = {}
    for i, column in enumerate(description):
        column_name = column[0]  # DuckDB description is tuple (name, type, ...)
        value = row[i]
...
def _get_duckdb_connection(driver_config: Dict[str, Any]) -> duckDB.DuckDBPyConnection:
    """
    Get DuckDB connection.
    ...
    """
    # Check if using file path or memory
    file_path = driver_config.get(Constants.FILE_PATH)
    use_memory = driver_config.get(Constants.MEMORY, True)
    ...
    if file_path:
        ...
        # Default to memory if neither specified
        return duckDB.connect()


def _test_duckdb_connection(
    connection: duckDB.DuckDBPyConnection, driver_config: Dict[str, Any]
) -> str:
    """Test DuckDB connection."""
    try:
        ...
        if result and result[0] == 1:
            ...
            return "DuckDB connection test failed"
    except Exception as e:
        return f"DuckDB connection failed: {str(e)}"
