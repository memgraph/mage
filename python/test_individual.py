...
def conflict_individual(graph):
    indv = Individual(no_of_colors=3, graph=graph, chromosome=[1, 1, 1, 2, 0], conflict_nodes={0, 1, 2})
    return indv
...
@pytest.mark.parametrize(
    ...
    [
        (3, graph(), [1, 1, 1, 2, 0], [2], [0], False, [1, 1, 0, 2, 0], {0, 1}, 2, False),
        (3, graph(), [1, 2, 0, 2, 1], [6], [0], True, None, None, None, False),
        (3, graph(), [1, 1, 0, 2, 0], [1, 3], [2, 0], False, [1, 2, 0, 0, 0], {2,3}, 3, False),
        (3, graph_not_connected(), [1, 1, 1, 2, 0], [2], [7], True, None, None, None, False),
        (3, graph_not_connected(), [1, 2, 0, 2, 1], [0], [2], False, [2, 2, 0, 2, 1], {0, 1}, 2, False),
        (3, graph_not_connected(), [1, 1, 0, 2, 2], [0,3], [2,1], False, [2, 1, 0, 1, 2], set(), 0, True),
        (3, graph_not_connected(), [1, 2, 0, 2, 2], [0], [2], False, [2, 2, 0, 2, 2], {0, 1, 3, 4}, 5, False)
    ])
def test_replace_units(no_of_colors, graph, chromosome, inds, colors,
        is_none, expected_chromosome, expected_conflicts, expected_weights, expected_coloring):
    indv = Individual(no_of_colors=no_of_colors, graph=graph, chromosome=chromosome)
