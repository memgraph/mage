...
def get_schema(
    ...
    for node in context.graph.vertices:
        ...
        _update_counts(node_count_by_labels, key=labels, obj=node, include_properties=include_properties)
...
def _iter_edges_as_map(
    ...
    for i, ((source_label, edge_label, target_label), counter) in enumerate(edge_count_by_labels.items()):
