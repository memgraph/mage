...
void LouvainProc(mgp_list *args, mgp_graph *graph, mgp_result *result, mgp_memory *memory) {
  try {
    ...
    auto cu_graph = mg_cugraph::CreateCugraphFromMemgraph<false, false>(*mg_graph.get(), handle);
    auto cu_graph_view = cu_graph.view();
    auto n_vertices = cu_graph_view.get_number_of_vertices();

    rmm::device_uvector<mg_cugraph::vertex_t> clustering_result(n_vertices, stream);
    cugraph::louvain(handle, cu_graph_view, clustering_result.data(), max_level, mg_cugraph::weight_t{1});

    for (mg_cugraph::vertex_t node_id = 0; node_id < clustering_result.size(); ++node_id) {
