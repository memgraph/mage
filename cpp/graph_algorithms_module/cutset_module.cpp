...
bool InsertCutsetRecord(const mgp_graph *graph, mgp_result *result,
                        mgp_memory *memory, const int cutset_id,
                        const int edge_id, const int node_from_id,
                        const int node_to_id) {
  mgp_result_record *record = mgp_result_new_record(result);
  if (record == nullptr) return false;

  bool result_inserted =
      mg_interface::InsertIntValue(record, fieldCutsetID, cutset_id, memory);
...
void GetCutSets(const mgp_list *args, const mgp_graph *graph,
  ...
  try {
    ...
    mg_interface::GetGraphView(&g, node_mapping, edge_mapping, graph, result,
                               memory);
    std::vector<std::vector<graphdata::Edge>> cutsets = algorithms::GetCutsets(g);

    for (uint32_t cutset_id = 0; cutset_id < cutsets.size(); cutset_id++) {
      for (const auto &edge : cutsets[cutset_id]) {
        bool record_inserted =
            InsertCutsetRecord(graph, result, memory, cutset_id,
                               edge_mapping[edge.id], edge.from, edge.to);
...
extern "C" int mgp_init_module(mgp_module *module, mgp_memory *memory) {
  mgp_proc *proc = mgp_module_add_read_procedure(module, "cutsets", GetCutSets);
