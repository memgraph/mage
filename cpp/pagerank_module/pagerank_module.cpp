 ...
 * measuring influence of connected nodes.
 *
...
static void ParallelPagerank(const mgp_list *args, const mgp_graph *graph,
                             mgp_result *result, mgp_memory *memory, bool pattern) {
    ...
    try {
        ...
        if (!pattern) {
            ...
            graphMapping = MapMemgraphGraphWithPatterns(nodes, patterns, idToIter, graph, memory);
        }
...
//         Make Pagerank graph
        ...
        for (uint32_t i = 0; i < number_of_nodes; ++i) {
            ...
            int value_inserted = mgp_result_record_insert(record, "node", vertex_value);
...
extern "C" int mgp_init_module(struct mgp_module *module,
    ...
    struct mgp_proc *subgraph_proc =
            mgp_module_add_read_procedure(module, "pattern_pagerank", PatternPagerankWrapper);

    if (!subgraph_proc) return 1;
    if (!mgp_proc_add_opt_arg(subgraph_proc, "transform_nodes", mgp_type_list(mgp_type_node()),
                              mgp_value_make_list(mgp_list_make_empty(0, memory))))
        return 1;

    if (!mgp_proc_add_opt_arg(subgraph_proc, "transform_patterns", mgp_type_list(mgp_type_list(mgp_type_string())),
                              mgp_value_make_list(mgp_list_make_empty(0, memory))))
        return 1;
