...
void Path::FilterLabel(const std::string_view label, const std::unordered_set<std::string> &termination_list,
                       const std::unordered_set<std::string> &blacklist,
                       const std::unordered_set<std::string> &whitelist,
                       const std::unordered_set<std::string> &end_list, bool &blacklisted, bool &terminated,
                       bool &end_node, bool &whitelisted) {
...
void Path::ParseLabels(const mgp::List &list_of_labels, std::unordered_set<std::string> &termination_list,
...
void Path::Parse_Relationships(const mgp::List &list_of_relationships, std::unordered_set<std::string> &outgoing_rel,
...
void Path::Path_DFS(mgp::Path path, std::unordered_set<mgp::Relationship> relationships_set,
                    ...
                    const std::unordered_set<std::string> &termination_list,
                    const std::unordered_set<std::string> &blacklist, const std::unordered_set<std::string> &whitelist,
                    const std::unordered_set<std::string> &end_list, const bool &end_node_activated,
                    const bool &whitelist_empty, const bool &termination_activated,
  ...
  for (auto rel : node.OutRelationships()) {
    ...
    if (path_size > 0 &&
        rel == path.GetRelationshipAt(
                   path_size -
                   1)) {  // to prevent backtracking the same way path came from, and starting a loop of back and forth
      continue;
    }
    ...
    mgp::Path cpy = mgp::Path(path);  // I make a copy here because I need to store every path and sub-path as a
                                      // separate result, feel free to give me a better idea :)
    std::string rel_type = std::string(rel.Type());
    if (!any_outgoing && (outgoing_rel.find(rel_type) == outgoing_rel.end()) &&
        (any_rel.find(rel_type) ==
         any_rel.end())) {  // check if relationship is allowed or all relationships are allowed
      continue;
    }
    cpy.Expand(rel);  // expand the path with this relationships
    std::unordered_set<mgp::Relationship> relationships_set_cpy = relationships_set; /* NOTE TO REWIEVER- HERE, I ALSO
    MAKE A COPY BECAUSE EVERY PATH AND SUBPATH HAVE THEIR OWN RELATIONSHIPS VISITED, BUT I FEEL THIS CAN BE DONE BETTER,
    BUT WHEN I DO IT WITHOUT THE COPY IT DOESNT WORK AS INTENDED, THOUGHTS?*/
    ...
    for (auto label : rel.To().Labels()) {  // set booleans to their value for the label of the finish node
      FilterLabel(label, termination_list, blacklist, whitelist, end_list, blacklisted, terminated, end_node,
                  whitelisted);
    }

    /*if path is inside specified size, and the node to which we are expanding is not blacklisted
    or it is an end node(end_node_activated is se to true at the beggining of the procedure if there is end nodes, not
    sure if it is needed)

    or the node is a termination node

    or if there are no termination nodes(termination_activated == false => this is neccesary because if there are
    specified termination labels, only paths ending in them can be returned) and the node is either whitelisted or the
    whitelist is empty(which means everything is whitelisted)
    ...
    */
    if ((path_size + 1 <= max_hops) && (path_size + 1 >= min_hops) && !blacklisted &&
        ((end_node && end_node_activated) || terminated ||
         (!termination_activated && !end_node_activated && (whitelisted || whitelist_empty)))) {
      path_list.push_back(cpy);  // save the path in result vector
    }

    /*if node is not blacklisted, or termination node and if it is an end node or in the whitelist, it can be
     * expandend*/
    if (path_size + 1 < max_hops && !blacklisted && !terminated && (end_node || (whitelisted || whitelist_empty))) {
      Path_DFS(cpy, relationships_set_cpy, path_list, path_size + 1, min_hops, max_hops, termination_list, blacklist,
               whitelist, end_list, end_node_activated, whitelist_empty, termination_activated, outgoing_rel,
               incoming_rel, any_rel, any_outgoing, any_incoming);
    }
  }
  // incoming rels
  // the code here is similar as above, only few things are changed to accomdodate for INRelationship type, so the
  // comments and notes are the saem
  for (auto rel : node.InRelationships()) {
    ...
    std::string rel_type = std::string(rel.Type());
    if (!any_incoming && (incoming_rel.find(rel_type) == incoming_rel.end()) &&
        (any_rel.find(rel_type) == any_rel.end())) {  // check if rel allowed
      continue;
    }
    cpy.Expand(rel);
...
void Path::Expand(mgp_list *args, mgp_graph *memgraph_graph, mgp_result *result, mgp_memory *memory) {
  ...
  try {
    ...
    if (end_list.size() != 0) {  // end node is activated, which means only paths ending with it can be saved as result,
    ...
    Path_DFS(path, relationships_set, path_list, 0, min_hops, max_hops, termination_list, blacklist, whitelist,
             end_list, end_node_activated, whitelist_empty, termination_activated, outgoing_rel, incoming_rel, any_rel,
