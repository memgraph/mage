...
/// Calculating summation of dangle nodes ranks to use to in personalise page rank
/// @param graph -- graph
/// @param start_rank -- rank of initial nodes
        double DangleNodeSum(const PageRankGraph &graph, std::vector<double> start_rank) {
            std::vector<std::uint64_t> is_dangling_nodes = graph.GetDangleNodes();
            double sum_dangle_node_rank = 0;
            for (std::uint64_t dangle_node_id = 0; dangle_node_id < is_dangling_nodes.size(); dangle_node_id++) {
                sum_dangle_node_rank += start_rank[is_dangling_nodes[dangle_node_id]];
            }
            return sum_dangle_node_rank;
        }

/// Calculating personalised pagerank block related to [lo, hi> interval of edges
...
/// @param new_rank_promise -- used for sending information about calculated new block
        void ThreadPageRankIteration(const PageRankGraph &graph, const std::vector<double> &old_rank,
...
/// @param rank_next -- personalised pagerank which will be updated
        void AddCurrentBlockToRankNext(const PageRankGraph &graph, const double damping_factor,
                                       ...
                                       std::vector<double> &dangle_p_rank) {
            for (std::size_t node_index = 0; node_index < block.size(); node_index++) {
                rank_next[node_index] += damping_factor * (block[node_index] + dangle_p_rank[node_index]);
            }
        }
...
std::vector<double> ParallelIterativePPageRank(const PageRankGraph &graph, const std::vector<std::pair<int, double>>& personalisation,
    ...
    std::vector<double> start_rank(graph.GetNodeCount(), 1.0 / graph.GetNodeCount());
    std::vector<double> prank(graph.GetNodeCount(), 1.0 / graph.GetNodeCount());
    if (!personalisation.empty()) {
        std::vector<double> personalised_rank(graph.GetNodeCount(), 0);
        for (const auto [node, p_value]: personalisation) {
            personalised_rank[node] = p_value;
        }
        const double sum = std::accumulate(start_rank.begin(), start_rank.end(), 0.0);
        for (double &value: personalised_rank) { value /= sum; }
        prank.swap(personalised_rank);
    }
    bool continue_iterate = max_iterations != 0;
    std::size_t number_of_iterations = 0;
    while (continue_iterate) {
        double sum_dangle_node_rank = DangleNodeSum(graph, start_rank);
        std::vector<double> dangle_p_rank = prank;
        std::transform(dangle_p_rank.begin(), dangle_p_rank.end(), dangle_p_rank.begin(),
                       [sum_dangle_node_rank](double &c) { return c * sum_dangle_node_rank; });
        ...
        for (std::size_t cluster_id = 0; cluster_id < number_of_threads; cluster_id++) {
            my_threads.emplace_back(
                    [&, lo = borders[cluster_id], hi = borders[cluster_id + 1]](
                            auto promise) { ThreadPageRankIteration(graph, start_rank, lo, hi, std::move(promise)); },
                    std::move(page_rank_promise[cluster_id])
            );
        }
        std::vector<double> rank_next = prank;
        double damp = 1 - damping_factor;
        std::transform(dangle_p_rank.begin(), dangle_p_rank.end(), dangle_p_rank.begin(),
                       [damp](double &c) { return c * damp; });
        for (std::size_t cluster_id = 0; cluster_id < number_of_threads; cluster_id++) {
            std::vector<double> block = page_rank_future[cluster_id].get();
            AddCurrentBlockToRankNext(graph, damping_factor, block, rank_next, dangle_p_rank);
        }
        ...
        start_rank.swap(rank_next);
        number_of_iterations++;
        continue_iterate = CheckContinueIterate(start_rank, rank_next, max_iterations, stop_epsilon,
                                                number_of_iterations);


    }
    NormalizeRank(start_rank);
    return start_rank;
