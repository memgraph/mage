...
TEST(BipartiteMatching, RandomCompleteBipartiteGraphs) {
  ...
  for (int t = 0; t < num_of_experiments; ++t) {
    ...
    auto max_match = bipartite_matching_util::MaximumMatching(edges);
    ASSERT_EQ(max_match, std::min(size_a, size_b));
  }
}

TEST(BipartiteMatching, NotBipartiteGraph) {
  auto g = mg_generate::BuildGraph(5, {{0, 1}, {0, 2}, {0, 3}, {0, 4}, {1, 2}, {1, 3}, {1, 4}, {2, 3}, {2, 4}, {3, 4}});
  auto is_graph_bipartite = bipartite_matching_util::IsGraphBipartite(*g);
  ASSERT_FALSE(is_graph_bipartite);
}

TEST(BipartiteMatching, BipartiteGraph) {
  auto g = mg_generate::BuildGraph(4, {{0, 1}, {1, 2}, {2, 3}, {3, 0}});
  auto is_graph_bipartite = bipartite_matching_util::IsGraphBipartite(*g);
  ASSERT_TRUE(is_graph_bipartite);
}

TEST(BipartiteMatching, BipartiteGraphWith2Components) {
  auto g = mg_generate::BuildGraph(5, {{0, 1}, {1, 2}, {3, 4}});
  auto is_graph_bipartite = bipartite_matching_util::IsGraphBipartite(*g);
  ASSERT_TRUE(is_graph_bipartite);
}

TEST(BipartiteMatching, NotBipartiteGraphWithSelfLoop) {
  auto g = mg_generate::BuildGraph(5, {{0, 1}, {1, 2}, {2, 3}, {0, 0}});
  auto is_graph_bipartite = bipartite_matching_util::IsGraphBipartite(*g);
  ASSERT_FALSE(is_graph_bipartite);
}
...
TEST(BipartiteMatching, HandmadeGraph1) {
  auto g = mg_generate::BuildGraph(10, {{0, 6}, {0, 7}, {1, 5}, {1, 8}, {2, 7}, {3, 7}, {3, 8}, {4, 9}});
...
TEST(BipartiteMatching, HandmadeGraph2) {
  auto g = mg_generate::BuildGraph(9, {{4, 6}, {0, 6}, {3, 7}, {2, 5}, {1, 6}, {3, 8}});
  auto max_match = bipartite_matching_alg::BipartiteMatching(*g);
  ASSERT_EQ(max_match, 3);
}
...
TEST(BipartiteMatching, HandmadeGraph3) {
  auto g = mg_generate::BuildGraph(10, {{0, 6}, {0, 9}, {1, 7}, {2, 6}, {3, 7}, {3, 8}, {4, 5}});
  auto max_match = bipartite_matching_alg::BipartiteMatching(*g);
  ASSERT_EQ(max_match, 5);
}
...
TEST(BipartiteMatching, HandmadeGraph5) {
  auto g = mg_generate::BuildGraph(20, {{0, 16}, {1, 10}, {1, 15}, {1, 17}, {2, 13}, {2, 14}, {2, 15}, {2, 16}, {3, 11},
