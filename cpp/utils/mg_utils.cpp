...
/// Convert Memgraph list to the edge patterns. Shared nodes are converted into
/// edges.
///
...
ListToPattern(const mgp_list *edge_patterns) {
  ...
  for (int i = 0; i < size; i++) {
    const auto *pattern_list =
        mgp_value_get_list(mgp_list_at(edge_patterns, i));
    ...
    for (int j = 0; j < pattern_size; j++) {
      ...
      pattern.emplace_back(std::move(label));
    }
...
std::set<int64_t> GetAdjacentVertexId(const mgp_vertex *start_vertex,
  ...
  if (edges_it_out == nullptr) {
    throw std::runtime_error("Not enough memory!");
  }
  utils::OnScopeExit edges_it_out_deleter([&edges_it_out] {
    if (edges_it_out != nullptr) {
      mgp_edges_iterator_destroy(edges_it_out);
    }
  });
  auto *edges_it_in = mgp_vertex_iter_in_edges(start_vertex, memory);
  if (edges_it_in == nullptr) {
    throw std::runtime_error("Not enough memory!");
  }
  utils::OnScopeExit edges_it_in_deleter([&edges_it_in] {
    if (edges_it_in != nullptr) {
...
std::set<int64_t> FindPattern(const std::vector<std::string> &pattern,
  ...
  for (const std::string_view label_name : pattern) {
    std::set<int64_t> next_vertices;

    for (const int64_t from_id : from_vertices) {
      const mgp_vertex *from = mgp_graph_get_vertex_by_id(
          graph, mgp_vertex_id{.as_int = from_id}, memory);
      std::set<int64_t> adjacentID = GetAdjacentVertexId(from, memory);

      for (const int64_t id : adjacentID) {
        const mgp_vertex *vertex = mgp_graph_get_vertex_by_id(
            graph, mgp_vertex_id{.as_int = id}, memory);
        const bool label_check =
            mgp_vertex_has_label_named(vertex, label_name.data());
        if (label_check)
          next_vertices.emplace(id);
      }
    }
    ...
    for (const int64_t v : next_vertices) {
  ...
  for (const int64_t from_id : from_vertices) {
    const mgp_vertex *from = mgp_graph_get_vertex_by_id(
        graph, mgp_vertex_id{.as_int = from_id}, memory);
    const auto adjacentID = GetAdjacentVertexId(from, memory);
...
GraphMapping MapMemgraphGraph(const std::map<uint32_t, uint32_t> &idToIter,
  ...
  for (const auto *vertex = mgp_vertices_iterator_get(vertices_it); vertex;
    ...
    for (const auto *out_edge = mgp_edges_iterator_get(edges_it); out_edge;
      ...
      const int64_t first_node_id = mgp_vertex_get_id(from).as_int;
      const int64_t second_node_id = mgp_vertex_get_id(to).as_int;

      const uint32_t first_node_map = idToIter.at(first_node_id);
      const uint32_t second_node_map = idToIter.at(second_node_id);

      // Make list of edges.
      edges.emplace_back(first_node_map, second_node_map);
      NE++;
    }
...
GraphMapping MapMemgraphGraphWithPatterns(
  ...
  for (int i = 0; i < nodes_size; i++) {
    ...
    for (const auto &pattern : patterns) {
      std::set<int64_t> to_vertices =
          FindPattern(pattern, from, memgraphGraph, memory);
      ...
      for (int64_t to_id : to_vertices) {
        const int64_t first_node_id = mgp_vertex_get_id(from).as_int;
        const int64_t second_node_id = to_id;

        const uint32_t first_node_map = idToIter.at(first_node_id);
        const uint32_t second_node_map = idToIter.at(second_node_id);

        // Make list of edges.
        if (first_node_map > second_node_map)
          continue;
        auto edge = std::make_pair(first_node_map, second_node_map);
        if (std::count(edges.begin(), edges.end(), edge))
          continue;
...
PairMap<uint32_t, uint32_t> VertexIdMappingSubgraph(const mgp_list *nodes,
  ...
  const int size = mgp_list_size(nodes);
  for (int i = 0; i < size; i++) {
    const auto *vertex = mgp_value_get_vertex(mgp_list_at(nodes, i));
    const uint32_t vertexId = mgp_vertex_get_id(vertex).as_int;

    iterToId.emplace(vertexIter, vertexId);
    idToIter.emplace(vertexId, vertexIter);
    vertexIter++;
  }
