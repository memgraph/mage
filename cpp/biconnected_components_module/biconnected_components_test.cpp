...
/// directions.
inline std::unique_ptr<mg_graph::Graph<>> BuildGraph(std::uint64_t nodes,
...
inline std::unique_ptr<mg_graph::Graph<>> GenRandomGraph(std::uint64_t nodes, std::uint64_t edges) {
  ...
  return BuildGraph(nodes, std::vector<std::pair<std::uint64_t, std::uint64_t>>(E.begin(), E.end()));
}
...
inline std::unique_ptr<mg_graph::Graph<>> GenRandomTree(std::uint64_t nodes) {
  auto seed = std::chrono::high_resolution_clock::now().time_since_epoch().count();
  std::mt19937 rng(seed);
  std::vector<std::pair<std::uint64_t, std::uint64_t>> edges;
  for (std::uint64_t i = 1; i < nodes; ++i) {
    std::uniform_int_distribution<std::uint64_t> dist(0, i - 1);
    std::uint64_t dad = dist(rng);
...
TEST(BCC, EmptyGraph) {
  auto G = BuildGraph(0, {});
  auto BCC = bcc_algorithm::GetBiconnectedComponents(G.get());
