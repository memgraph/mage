...
struct Graph {
    ...
    ///
    bool IsVertexInGraph(std::uint64_t u) const {
...
struct Partitions {
    ...
    ///
    std::uint64_t GetCommunityForNode(std::uint64_t node_id) const {
    ...
    ///
    std::uint64_t GetCommunityWeight(std::uint64_t community_id) const {
