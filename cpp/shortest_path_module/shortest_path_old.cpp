...
static std::unique_ptr<mgp_vertex, VertexDelete> CreateVertexPointer(mgp_vertex *v, mgp_memory *memory) {
  return std::unique_ptr<mgp_vertex, VertexDelete>(mgp::vertex_copy(v, memory));
}

static std::unique_ptr<mgp_edge, EdgeDelete> CreateEdgePointer(mgp_edge *e, mgp_memory *memory) {
  return std::unique_ptr<mgp_edge, EdgeDelete>(mgp::edge_copy(e, memory));
}

struct PriorityPathItem {
  std::int32_t distance;
  std::unique_ptr<mgp_vertex, VertexDelete> vertex;
  std::vector<std::unique_ptr<mgp_edge, EdgeDelete>> path;
...
void InsertPathResult(mgp_graph *graph, mgp_result *result, mgp_memory *memory, mgp_vertex *source, mgp_vertex *target,
                      const std::vector<std::unique_ptr<mgp_edge, EdgeDelete>> &edges) {
...
void ShortestPath(mgp_list *args, mgp_graph *memgraph_graph, mgp_result *result, mgp_memory *memory) {
  try {
    ...
    for (auto target : targets) {
      ...
      priority_queue.push(
          {0, CreateVertexPointer(target, memory), std::vector<std::unique_ptr<mgp_edge, EdgeDelete>>{}});
