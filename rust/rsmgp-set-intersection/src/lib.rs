...
init_module!(|memgraph: &Memgraph| -> Result<()> {
    ...
    memgraph.add_read_procedure(
        is_set_subset,
        c_str!("is_set_subset"),
        &[  define_type!("set", Type::List, Type::Map), 
            define_type!("subset", Type::List, Type::Map)
        ],
        &[],
        &[define_type!("is_subset", Type::Bool)],
    )?;
...
define_procedure!(is_set_subset, |memgraph: &Memgraph| -> Result<()> {
    let result = memgraph.result_record()?;
    let args = memgraph.args()?;

    let set_value = args.value_at(0)?;
    let subset_value = args.value_at(1)?;

    if let Value::List(set) = set_value {
        if let Value::List(subset) = subset_value {
            let mut is_subset = true;

            let subset_iter = subset.iter()?;
            for sub_value in subset_iter {
                let mut found = false;

                let set_iter = set.iter()?;
                for set_value in set_iter {
                    if sub_value == set_value {
                        found = true;
                        break;
                    }
                }
            ...
            result.insert_bool(c_str!("is_subset"), is_subset)?;
        }
    }
